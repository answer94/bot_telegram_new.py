...
async def check_birthday():
    ...
    for i in hb_keys:
        ...
        if datetime_object1.strftime('%d.%m') == date_hb.strftime('%d.%m'):
            await bot.send_message(-662196356,(f"День Рождения у {staff_data_hb[str(i)]} \n Не забудьте поздравить {datetime_object1.strftime('%d.%m')}."))
...
async def bot_state8(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        file_otpusk = wotpusk.write_word_holiday(df_otpusk, data['ФИО'], data["all"], data["number_day"], data["date"],
                                            dt_now)
